...
class TrainModel(object):
    def __init__(self, is_test, args):
        self.args = args
        if is_test:
            full_trainloader = data_loader.get_train_loader('../datasets/CIFAR10_data', batch_size=self.args.batch_size, augment=True,shuffle=True, random_seed=1000, show_sample=False,num_workers=4, pin_memory=True)
            testloader = data_loader.get_test_loader('../datasets/CIFAR10_data', batch_size=self.args.batch_size, shuffle=False,num_workers=4, pin_memory=True)
            self.full_trainloader = full_trainloader
            self.testloader = testloader
        else:
            trainloader, validate_loader = data_loader.get_train_valid_loader('../datasets/CIFAR10_data', batch_size=self.args.batch_size_search, augment=True, subset_size=1, valid_size=0.1, shuffle=True,random_seed=1000, show_sample=False,num_workers=4, pin_memory=True)
